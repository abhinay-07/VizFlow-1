...
class CleanerService {
  ...
  cleanRecord(record, rowIndex) {
    ...
    for (const [key, value] of Object.entries(record)) {
      ...
      if (
        ...
        this.logError(
          ...
          cleanedKey,
          value,
          `Required field '${cleanedKey}' is missing or empty`,
          rowIndex,
          "high"
        );
      }
    ...
    const customerErrors = this.validateCustomerData(cleanedRecord, rowIndex);
    customerErrors.forEach((error) => this.errors.push(error));
  ...
  cleanFieldValue(value, fieldName, rowIndex) {
    ...
    try {
      ...
      this.logError(
        ...
        fieldName,
        value,
        `Failed to clean value: ${error.message}`,
        rowIndex,
        "medium"
      );
  ...
  validateCustomerData(record, rowIndex) {
    ...
    const amountFields = ["amount", "credit_limit", "outstanding_balance"];
    ...
    if (
      ...
      !["credit", "debit", "transfer", "payment", "withdrawal"].includes(
      ...
      customerErrors.push({
        ...
        message:
          "Invalid transaction type. Must be: credit, debit, transfer, payment, or withdrawal",
        rowIndex,
        severity: "low",
      });
    }
